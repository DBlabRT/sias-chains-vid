...
#define HEAP_SIAS_DELETED		0x0010	/* tuple has been deleted */
...
//TODO testing
#define HEAP_SIAS_RELOCATED		0x2000
...
#define HeapTupleHeaderGetVid(tup) \
( \
	(tup)->t_choice.t_heap.t_vid \
)

#define HeapTupleHeaderSetVid(tup, vid) \
( \
	(tup)->t_choice.t_heap.t_vid = (vid) \
)
...
#define HeapTupleHeaderIsSiasDeleted(tup) \
( \
	((tup)->t_infomask & HEAP_SIAS_DELETED) != 0 \
)

#define HeapTupleHeaderSetSiasDeleted(tup) \
( \
	(tup)->t_infomask |= HEAP_SIAS_DELETED \
)

#define HeapTupleHeaderClearSiasDeleted(tup) \
( \
	(tup)->t_infomask &= ~HEAP_SIAS_DELETED \
)

#define HeapTupleHeaderIsSiasRelocated(tup) \
( \
  (tup)->t_infomask2 & HEAP_SIAS_RELOCATED \
)

#define HeapTupleHeaderSetSiasRelocated(tup) \
( \
  (tup)->t_infomask2 |= HEAP_SIAS_RELOCATED \
)
...
typedef struct HeapTupleData
{
	...
	ItemPointerData t_entryPoint;	/* SIAS entry point of the corresponding
									   tuple chain */
	ItemVid t_vid;					/* reference VID to entry point */
