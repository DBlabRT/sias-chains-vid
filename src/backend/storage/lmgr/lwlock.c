...
NumLWLocks(void)
{
	...
	/* SIAS: per user relation: 2 per bucket buffer, 1 per buffer partition, 1 bufFreelistLock, 1 appendLock,
	 * 							(5 + SIAS_CONCURRENT_TARGET_BLOCKS) for TargetBlockInfo  */
	//numLocks += SIAS_MAX_RELATIONS * (((SIAS_BUCKET_SIZE + 2) * SIAS_BUFFER_POOL_SIZE) + SIAS_BUFFER_POOL_PARTITIONS + 1);
	numLocks += SIAS_MAX_RELATIONS * ((2 * SIAS_BUFFER_POOL_SIZE) + SIAS_BUFFER_POOL_PARTITIONS + 1 + 1 +
									  (5 + SIAS_CONCURRENT_TARGET_BLOCKS));
