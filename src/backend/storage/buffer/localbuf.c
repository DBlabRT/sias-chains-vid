 ...
 */
BufferDesc *
LocalBufferAlloc(SMgrRelation smgr, bool isSystemRelation, ForkNumber forkNum, BlockNumber blockNum,
				 bool *foundPtr)
{
	BufferTag	newTag;			/* identity of requested block */
	LocalBufferLookupEnt *hresult;
...
//				fprintf(file, "cache local %u\n", bufHdr->tag.blockNum);
//				fclose(file);
				CacheSiasBlock(&(bufHdr->tag.rnode),
							   bufHdr->tag.blockNum,
	...
	bufHdr->tag = newTag;
#ifdef SIAS
	bufHdr->flags &= ~(BM_VALID | BM_DIRTY | BM_JUST_DIRTIED | BM_IO_ERROR | BM_SIAS_BUFFER | BM_SIAS_FULL);
	if (!isSystemRelation)
		bufHdr->flags |= BM_SIAS_BUFFER;
#else
