...
//			fprintf(file, "invalid page header: %u %u %u %u %u %u\n",
 ...
 * PageDeleteLastItem
 *
 * Removes the most recently added tuple
 * and its item pointer.
 *
 * Assumption:
 *
 * The last insertion was done at offset
 * PageGetMaxOffsetNumber().
 */
bool
PageDeleteLastItem(Page page)
{
	...
	offsetNum = PageGetMaxOffsetNumber(page);
	...
	alignedSize = MAXALIGN(ItemIdGetLength(itemId));
	phdr = (PageHeader) page;

	/*
	 * We can't handle cases in which the latest
	 * insertion wasn't done at offset
	 * PageGetMaxOffsetNumber().
	 */
	if (ItemIdGetOffset(itemId) != phdr->pd_upper)
	{
		return false;
	}
	 ...
	 * remove the tuple
	 */
	memset(PageGetItem(page, itemId), 0, alignedSize);
	phdr->pd_upper += (LocationIndex) alignedSize;

	/*
	 * remove the item pointer
	 */
	memset(itemId, 0, sizeof(ItemIdData));
	phdr->pd_lower -= (LocationIndex) sizeof(ItemIdData);

	return true;
}

void
PageAddVidTidPair(Page page, Item item, OffsetNumber offsetNumber)
{
	...
	memcpy((char *) (page + lower + (SIAS_BUCKET_ENTRY_SIZE * (offsetNumber - 1))),
			item, SIAS_BUCKET_ENTRY_SIZE);
}
