...
preprocess_targetlist(PlannerInfo *root, List *tlist)
{
	...
	{
		TargetEntry *tleSelfPointer, *tleEntryPointPointer, *tleMaxTransactionId, *tleMaxCommandId, *tleVid;
		Var		   *varSelfPointer, *varEntryPointPointer, *varMaxTransactionId, *varMaxCommandId, *varVid;

		varSelfPointer = makeVar(result_relation, SelfItemPointerAttributeNumber,
					TIDOID, -1, 0);
		varEntryPointPointer = makeVar(result_relation, EntryPointAttributeNumber,
					TIDOID, -1, 0);
		varMaxTransactionId =  makeVar(result_relation, MaxTransactionIdAttributeNumber,
					XIDOID, -1, 0);
		varMaxCommandId =  makeVar(result_relation, MaxCommandIdAttributeNumber,
					CIDOID, -1, 0);
		varVid =  makeVar(result_relation, VidAttributeNumber,
					CIDOID, -1, 0);

		tleSelfPointer = makeTargetEntry((Expr *) varSelfPointer,
							  list_length(tlist) + 1,
							  pstrdup("ctid"),
							  true);
		tleEntryPointPointer = makeTargetEntry((Expr *) varEntryPointPointer,
									  list_length(tlist) + 2,
									  pstrdup("entrypoint"),
									  true);
		tleMaxTransactionId = makeTargetEntry((Expr *) varMaxTransactionId,
									  list_length(tlist) + 3,
									  pstrdup("xmax"),
									  true);
		tleMaxCommandId = makeTargetEntry((Expr *) varMaxCommandId,
									  list_length(tlist) + 4,
									  pstrdup("cmax"),
									  true);
		tleVid = makeTargetEntry((Expr *) varVid,
											  list_length(tlist) + 5,
											  pstrdup("vid"),
											  true);
		...
		tlist = lappend(tlist, tleSelfPointer);
		tlist = lappend(tlist, tleEntryPointPointer);
		tlist = lappend(tlist, tleMaxTransactionId);
		tlist = lappend(tlist, tleMaxCommandId);
		tlist = lappend(tlist, tleVid);
	}
	...
	foreach(lc, root->rowMarks)
	{
		...
		if (rc->markType != ROW_MARK_COPY)
		{
			...
			var = makeVar(rc->rti,
						  EntryPointAttributeNumber,
						  TIDOID,
						  -1,
						  0);
			snprintf(resname, sizeof(resname), "entrypoint%u", rc->rowmarkId);
			tle = makeTargetEntry((Expr *) var,
								  list_length(tlist) + 1,
								  pstrdup(resname),
								  true);
			...
			var = makeVar(rc->rti,
						  ...
						  -1,
						  0);
			snprintf(resname, sizeof(resname), "xmax%u", rc->rowmarkId);
			tle = makeTargetEntry((Expr *) var,
								  list_length(tlist) + 1,
								  pstrdup(resname),
								  true);
			...
			var = makeVar(rc->rti,
						  ...
						  -1,
						  0);
			snprintf(resname, sizeof(resname), "cmax%u", rc->rowmarkId);
			tle = makeTargetEntry((Expr *) var,
								  list_length(tlist) + 1,
								  pstrdup(resname),
								  true);
			...
			var = makeVar(rc->rti,
						  ...
						  -1,
						  0);
			snprintf(resname, sizeof(resname), "vid%u", rc->rowmarkId);
			tle = makeTargetEntry((Expr *) var,
								  list_length(tlist) + 1,
								  pstrdup(resname),
								  true);
