...
typedef struct ResourceOwnerData
{
	...
	/* We have built-in support for remembering owned bucket buffers */
	int				nbucketbuffers;		/* number of owned bucket buffer pins */
	BucketBufferId	*bucketbuffers;		/* dynamically allocated array */
	int				maxbucketbuffers;	/* currently allocated array size */
							 ...
							 bool isTopLevel)
{
	...
	if (phase == RESOURCE_RELEASE_BEFORE_LOCKS)
	{
		...
		/*
		 * Release bucket buffer pins.  Note that ReleaseBucketBuffer will
		 * remove the bucket buffer entry from my list, so I just have to
		 ...
		 * issue warnings.	In the abort case, just clean up quietly.
		 */
		while (owner->nbucketbuffers > 0)
		{
			BucketBufferId *bucketBufferId;
			bucketBufferId = &(owner->bucketbuffers[owner->nbucketbuffers - 1]);
			if (isCommit)
				PrintBucketBufferLeakWarning(*bucketBufferId);
			ReleaseBucketBuffer(bucketBufferId->vmi, bucketBufferId->buffer);
		}
...
ResourceOwnerDelete(ResourceOwner owner)
{
	...
	if (owner->buffers)
		pfree(owner->buffers);
	if (owner->bucketbuffers)
		pfree(owner->bucketbuffers);
	if (owner->catrefs)
		pfree(owner->catrefs);
 ...
 */
void
ResourceOwnerEnlargeBucketBuffers(ResourceOwner owner)
{
	int			newmax;

	if (owner == NULL ||
		owner->nbucketbuffers < owner->maxbucketbuffers)
		return;					/* nothing to do */

	if (owner->bucketbuffers == NULL)
	{
		newmax = 16;
		owner->bucketbuffers = (BucketBufferId *)
			MemoryContextAlloc(TopMemoryContext, newmax * sizeof(BucketBufferId));
		owner->maxbucketbuffers = newmax;
	}
	else
	{
		newmax = owner->maxbucketbuffers * 2;
		owner->bucketbuffers = (BucketBufferId *)
			repalloc(owner->bucketbuffers, newmax * sizeof(BucketBufferId));
		owner->maxbucketbuffers = newmax;
	}
}

/*
 * Remember that a bucket buffer pin is owned by a ResourceOwner
 *
 * Caller must have previously done ResourceOwnerEnlargeBucketBuffers()
 *
 * We allow the case owner == NULL because the bufmgr is sometimes invoked
 * outside any transaction (for example, during WAL recovery).
 */
void
ResourceOwnerRememberBucketBuffer(ResourceOwner owner, VidMappingInfo *vmi, Buffer buffer)
{
	if (owner != NULL)
	{
		Assert(owner->nbucketbuffers < owner->maxbucketbuffers);
		owner->bucketbuffers[owner->nbucketbuffers].vmi = vmi;
		owner->bucketbuffers[owner->nbucketbuffers].buffer = buffer;
		owner->nbucketbuffers++;
	}
}

/*
 * Forget that a bucket buffer pin is owned by a ResourceOwner
 *
 * We allow the case owner == NULL because the bufmgr is sometimes invoked
 * outside any transaction (for example, during WAL recovery).
 */
void
ResourceOwnerForgetBucketBuffer(ResourceOwner owner, VidMappingInfo *vmi, Buffer buffer)
{
	if (owner != NULL)
	{
		BucketBufferId	   	*bucketbuffers = owner->bucketbuffers;
		int					nbb1 = owner->nbucketbuffers - 1;
		int					i;

		/*
		 * Scan back-to-front because it's more likely we are releasing a
		 * recently pinned bucket buffer. This isn't always the case of course, but
		 * it's the way to bet.
		 */
		for (i = nbb1; i >= 0; i--)
		{
			if (bucketbuffers[i].vmi->userRelationId == vmi->userRelationId
					&& bucketbuffers[i].buffer == buffer)
			{
				// this original approach was quite inefficient
				/*while (i < nbb1)
				{
					bucketbuffers[i] = bucketbuffers[i + 1];
					i++;
				}*/
				// this new approach should do it better,
				// although ordering has changed, but this
				// should be neglectable
				bucketbuffers[i] = bucketbuffers[nbb1];
				owner->nbucketbuffers = nbb1;
				return;
			}
		}
		elog(ERROR, "bucket buffer %d %d is not owned by resource owner %s",
				vmi->userRelationId, buffer, owner->name);
	}
}
