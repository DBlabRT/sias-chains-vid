...
#ifdef SIAS
	/*
	 *  allocate all SIAS-related data structures
	 */
	if (!IsSiasSystemRelation(relation)) {
		RelationAllocateVidMappingInfo(relation);
		RelationAllocateTargetBlockInfo(relation);
		RelationAllocateGarbageCollectionInfo(relation);
	}
#endif
		  ...
		  int natts, const FormData_pg_attribute *attrs)
{
	...
	/*
	 *  allocate invalidation vector
	 */
	//RelationAllocateInvalidationVector(relation);
						   ...
						   char relkind)
{
	...
	if (!IsSystemRelation(rel)
		...
		RelationAllocateVidMappingInfo(rel);

		/*
		 * initialize bucket relation (rel->rd_vmi->bucketRelation)
		 */

		Relation pg_class_desc = heap_open(RelationRelationId, RowExclusiveLock);
		Oid relId2 = GetNewRelFileNode(reltablespace, pg_class_desc);
		heap_close(pg_class_desc, RowExclusiveLock);

		RelFileNode *rfn = &(rel->rd_vmi->bucketRelation->rd_node);
		rfn->spcNode = rel->rd_node.spcNode;
		rfn->dbNode = rel->rd_node.dbNode;
		rfn->relNode = relId2;

		SMgrRelation srel = smgropen(*rfn);

		smgrcreate(srel, MAIN_FORKNUM, false);


		printf("---new relation---\n");
		printf("user   relation: %s\n", relpath(rel->rd_node, MAIN_FORKNUM));
		printf("bucket relation: %s\n", relpath(*rfn, MAIN_FORKNUM));


		// initialize the bucket relation as far as
		// necessary to perform IO via buffer manager with it
		Relation bucketRelation = rel->rd_vmi->bucketRelation;
		bucketRelation->rd_smgr = srel;
		bucketRelation->rd_refcnt = 0;
		bucketRelation->rd_istemp = false;
		bucketRelation->rd_islocaltemp = false;
		bucketRelation->rd_isnailed = false;
		bucketRelation->rd_isvalid = false;
		// simulates RelationInitLockInfo
		bucketRelation->rd_lockInfo.lockRelId.relId = relId2;
		bucketRelation->rd_lockInfo.lockRelId.dbId = rfn->dbNode;



		/*
		 * initialize rel->rd_tbi
		 */
		RelationAllocateTargetBlockInfo(rel);

		/*
		 * initialize rel->rd_gci
		 */
		RelationAllocateGarbageCollectionInfo(rel);
...
void
printRelationIdCache()
{
	FILE *file;
	file = fopen("/home/richard/Documents/file1.txt", "a");
	fprintf(file, "RelationIdCache %p\n", RelationIdCache);
	fclose(file);
}
