			 ...
			 char *out_dbname)
{
	 ...
	 */
	if (bootstrap || IsAutoVacuumWorkerProcess() || IsSiasGcWorkerProcess() || IsSiasPersWorkerProcess())
	{
...
Oid
GetDatabaseOid(const char* name)
{
	HeapTuple	tuple;


	tuple = GetDatabaseTuple(name);
	if (!HeapTupleIsValid(tuple))
		ereport(FATAL,
				(errcode(ERRCODE_UNDEFINED_DATABASE),
				 errmsg("database \"%s\" does not exist", name)));

	return HeapTupleGetOid(tuple);
}

bool
DatabaseExists(const char* dbname)
{
	HeapTuple	tuple;
