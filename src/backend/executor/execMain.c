...
EvalPlanQual(EState *estate, EPQState *epqstate,
			 ...
			 ItemPointer tid, ItemPointer entryPoint,
			 TransactionId priorXmax)
{
	if (IsSiasSystemRelation(relation)) {
		return EvalPlanQualPostgres(estate, epqstate, relation, rti, tid, priorXmax);
	} else {
#ifdef SIAS
		return EvalPlanQualSias(estate, epqstate, relation, rti, tid, entryPoint, priorXmax);
#else
		return EvalPlanQualPostgres(estate, epqstate, relation, rti, tid, priorXmax);
#endif
	}
...
TupleTableSlot *
EvalPlanQualPostgres(EState *estate, EPQState *epqstate,
					 Relation relation, Index rti,
					 ItemPointer tid, TransactionId priorXmax)
{
	 ...
	 */
	copyTuple = EvalPlanQualFetchPostgres(estate, relation, LockTupleExclusive,
										  tid, priorXmax);
...
TupleTableSlot *
EvalPlanQualSias(EState *estate, EPQState *epqstate,
				 Relation relation, Index rti,
				 ItemPointer tid, ItemPointer entryPoint,
				 TransactionId priorXmax)
{
	 ...
	 * Get and lock the updated version of the row; if fail, return NULL.
	 */
	copyTuple = EvalPlanQualFetchSias(estate, relation, LockTupleExclusive,
									  tid, entryPoint, priorXmax);
...
{
	if (IsSiasSystemRelation(relation)) {
		return EvalPlanQualFetchPostgres(estate, relation, lockmode, tid, priorXmax);
	} else {
#ifdef SIAS
		return EvalPlanQualFetchSias(estate, relation, lockmode, tid, entryPoint, priorXmax);
#else
		return EvalPlanQualFetchPostgres(estate, relation, lockmode, tid, priorXmax);
#endif
	}
...
HeapTuple
EvalPlanQualFetchPostgres(EState *estate, Relation relation, int lockmode,
				  ItemPointer tid, TransactionId priorXmax)
{
	...
	for (;;)
	{
		...
		if (heap_fetch(relation, &SnapshotDirty, &tuple, &buffer, true, NULL))
		{
			 ...
			 */
			test = heap_lock_tuple_postgres(relation, &tuple, &buffer,
...
HeapTuple
EvalPlanQualFetchSias(EState *estate, Relation relation, int lockmode,
				  ItemPointer tid, ItemPointer entryPoint,
				  TransactionId priorXmax)
{
	HeapTuple	copyTuple = NULL;
	...
	InitDirtySnapshot(SnapshotDirty);
	tuple.t_self = *tid;
	tuple.t_entryPoint = *entryPoint;
	Assert (ItemPointerEquals(tid, entryPoint));
	for (;;)
	{
		Buffer		buffer;
		...
		if (heap_fetch(relation, &SnapshotDirty, &tuple, &buffer, true, NULL))
		{
			 ...
			 * This is a live tuple, so now try to lock it.
			 */
			test = heap_lock_tuple_sias(relation, &tuple, &buffer,
										&update_ctid, &update_xmax,
										estate->es_output_cid,
										lockmode, false);
			/* We now have two pins on the buffer, get rid of one */
			...
			switch (test)
			{
				...
				case HeapTupleMayBeUpdated:
					ItemPointerCopy(&(tuple.t_entryPoint), entryPoint); /* needed in case of EvalPlanQual during UPDATE/DELETE*/
					break;

				case HeapTupleUpdated:
					ReleaseBuffer(buffer);
					elog(ERROR, "Entry point updated which can't be true");
					return NULL;	/* keep compiler quiet */

				case HeapTupleSiasDeleted:
					/* tuple was deleted, so give up */
					ReleaseBuffer(buffer);
					return NULL;
		...
		if (tuple.t_data->t_infomask & HEAP_SIAS_DELETED) {
			/* deleted, so forget about it */
			ReleaseBuffer(buffer);
			return NULL;
		}
		...
		ReleaseBuffer(buffer);
		elog(ERROR, "Entry point updated which might be true here -> seems to be a bug.");
		return NULL;	/* keep compiler quiet */
	}
