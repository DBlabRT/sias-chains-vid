...
lnext:
	...
	foreach(lc, node->lr_arowMarks)
	{
		...
		/* fetch the tuple's entryPoint */
		datum = ExecGetJunkAttribute(slot, aerm->ctidAttNo + 1,
									 &isNull);
		/* shouldn't ever get a null result... */
		if (isNull)
			elog(ERROR, "entryPoint is NULL");
		tuple.t_entryPoint = *((ItemPointer) DatumGetPointer(datum));

		/* fetch the tuple's xmax */
		datum = ExecGetJunkAttribute(slot, aerm->ctidAttNo + 2,
														&isNull);
		/* shouldn't ever get a null result... */
		if (isNull)
			elog(ERROR, "xmax is NULL");
		tuple.t_xmax = (TransactionId) DatumGetTransactionId(datum);

		/* fetch the tuple's cmax */
		datum = ExecGetJunkAttribute(slot, aerm->ctidAttNo + 3,
										&isNull);
		/* shouldn't ever get a null result... */
		if (isNull)
			elog(ERROR, "cmax is NULL");
		tuple.t_cmax = (CommandId) DatumGetCommandId(datum);

		/* fetch the tuple's vid */
		datum = ExecGetJunkAttribute(slot, aerm->ctidAttNo + 4,
										&isNull);
		/* shouldn't ever get a null result... */
		if (isNull)
			elog(ERROR, "vid is NULL");
		tuple.t_vid = *((ItemVid*) DatumGetPointer(datum));
		...
		switch (test)
		{
			...
			case HeapTupleUpdated:
				...
				copyTuple = EvalPlanQualFetch(estate, erm->relation, lockmode,
											  &update_ctid, &(tuple.t_entryPoint),
											  update_xmax);
			...
			case HeapTupleSiasDeleted:
				elog(ERROR, "attempted to lock SIAS hint tuple (delete or lock tuple)");
				break;
