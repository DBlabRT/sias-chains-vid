 ...
 *
 * IDENTIFICATION
 *	  $PostgreSQL: pgsql/src/backend/access/nbtree/nbtinsert.c,v 1.178.4.1 2010/08/29 19:33:21 tgl Exp $
 *
 *-------------------------------------------------------------------------
 */
	...
	for (;;)
	{
		...
		if (offset <= maxoff)
		{
			...
			if (!ItemIdIsDead(curitemid))
			{
				 ...
				 */
#ifdef SIAS
				else if (IsSiasSystemRelation(heapRel) ? heap_hot_search(&htid, heapRel, &SnapshotDirty, &all_dead)
													   : heap_hot_search_sias(itup, curitup, heapRel, &SnapshotDirty, &all_dead))
#else
...
#endif
				{
					 ...
					 */
#ifdef SIAS
					if (IsSiasSystemRelation(heapRel))
					{
						htid = itup->t_tid;
					...
					{
						if (heap_hot_search_sias(NULL, itup, heapRel, SnapshotSelf, NULL))
						{
							/* Normal case --- it's still live */
						}
						else
