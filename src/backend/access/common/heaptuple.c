...
heap_getsysattr(HeapTuple tup, int attnum, TupleDesc tupleDesc, bool *isnull)
{
	...
	*isnull = false;
	...
	switch (attnum)
	{
		...
		case MaxTransactionIdAttributeNumber:
#ifdef SIAS
			result = TransactionIdGetDatum(tup->t_xmax);
#else
		...
		case MaxCommandIdAttributeNumber:
			 ...
			 */
#ifdef SIAS
			if (tup->t_cmax!= MaxCommandId)
				result = CommandIdGetDatum(tup->t_cmax);
			else
				result = CommandIdGetDatum(HeapTupleHeaderGetRawCommandId(tup->t_data));
#else
			result = CommandIdGetDatum(HeapTupleHeaderGetRawCommandId(tup->t_data));
		...
		case TableOidAttributeNumber:
			...
			break;
		case EntryPointAttributeNumber:
			result = PointerGetDatum(&(tup->t_entryPoint));
			break;
		case VidAttributeNumber:
			result = PointerGetDatum(&(tup->t_vid));
			break;
		default:
			elog(ERROR, "invalid attnum: %d", attnum);
...
heap_copytuple(HeapTuple tuple)
{
	...
	newTuple->t_entryPoint = tuple->t_entryPoint;
	newTuple->t_tableOid = tuple->t_tableOid;
	newTuple->t_xmax = tuple->t_xmax;
	newTuple->t_cmax = tuple->t_cmax;
...
heap_copytuple_with_tuple(HeapTuple src, HeapTuple dest)
{
	...
	dest->t_entryPoint = src->t_entryPoint;
	dest->t_tableOid = src->t_tableOid;
	dest->t_xmax = src->t_xmax;
	dest->t_cmax = src->t_cmax;
				...
				bool *isnull)
{
	...
	ItemPointerSetInvalid(&(tuple->t_entryPoint));
	tuple->t_tableOid = InvalidOid;
	tuple->t_xmax = InvalidTransactionId;
	tuple->t_cmax = MaxCommandId;
