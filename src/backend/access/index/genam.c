...
systable_recheck_tuple(SysScanDesc sysscan, HeapTuple tup)
{
	...
	if (sysscan->irel)
	{
		...
		result = HeapTupleSatisfiesVisibility(sysscan->heap_rel, tup, scan->xs_snapshot, scan->xs_cbuf, NULL);
	...
	else
	{
		...
		result = HeapTupleSatisfiesVisibility(sysscan->heap_rel, tup, scan->rs_snapshot, scan->rs_cbuf, NULL);
