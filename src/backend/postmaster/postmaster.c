...
#include "postmaster/siasPersistence.h"
...
typedef struct bkend
{
	...
	bool		is_siasgc;		/* is it a Sias garbage collection process? */
	bool		is_siaspers;	/* is it a Sias persistence process? */
...
static bool siasShmemRebuild = false;
...
typedef enum
{
	...
	PM_SIAS_STARTUP,			/* starting up SIAS, everything else has to wait */
	PM_SIAS_SHUTDOWN,			/* shutting down SIAS, everything else has to wait */
	PM_SIAS_SHUTDOWN_DONE		/* shutting down SIAS, everything else has to wait */
} PMState;
...
#define BACKEND_TYPE_SIASGC		0x0008	/* Sias garbage collection process */
#define BACKEND_TYPE_SIASPERS	0x0010	/* Sias garbage collection process */
...
static void StartSiasGarbageCollectionWorker(void);
static void StartSiasPersistenceWorker(bool startup);
...
PostmasterMain(int argc, char *argv[])
{
	...
	SetPersistencyConfigFileDir(DataDir);
	SetGcRunDurationFileDir(DataDir);
...
#ifdef SIAS_GC
				SignalSomeChildren(SIGTERM, BACKEND_TYPE_SIASGC);
#endif
		...
		case SIGINT:
					 ...
					 pmState == PM_HOT_STANDBY)
			{
				...
				SignalSomeChildren(SIGTERM,
								 BACKEND_TYPE_NORMAL | BACKEND_TYPE_AUTOVAC | BACKEND_TYPE_SIASGC);
				/* and the autovac launcher too */
				if (AutoVacPID != 0)
					signal_child(AutoVacPID, SIGTERM);
				/* and the walwriter too */
...
#define LOOPHEADER()
	...
	while (LOOPTEST())
	{
		...
		if (pid == StartupPID)
		{
			 ...
			 * startup SIAS
			 */
			StartSiasPersistenceWorker(true);
			pmState = PM_SIAS_STARTUP;
#else
			pmState = PM_RUN;
...
#ifdef SIAS_GC
				/*
				 * If there are regions left to be GC'ed
				 * from before the last shutdown or due
				 * to the last shutdown, we need to process
				 * them.
				 */
				StartSiasGarbageCollectionWorker();
#endif
			}
			else if (pmState == PM_SIAS_SHUTDOWN)
			{
				pmState = PM_SIAS_SHUTDOWN_DONE;

				/*
				 * PostmasterStateMachine() does the work
				 */
			}
			else
			{
				elog(PANIC, "incompatible pmState");
			}
...
PostmasterStateMachine(void)
{
	...
	if (pmState == PM_WAIT_BACKENDS)
	{
			...
			AutoVacPID == 0)
		{
			...
			else
			{
				 ...
				 * shutdown SIAS
				 */
				StartSiasPersistenceWorker(false);
				pmState = PM_SIAS_SHUTDOWN;
#else
	...
	if (pmState == PM_SIAS_SHUTDOWN_DONE)
	{
		pmState = PM_WAIT_BACKENDS;
...
SignalSomeChildren(int signal, int target)
{
	...
	for (curr = DLGetHead(BackendList); curr; curr = DLGetSucc(curr))
	{
		...
		if (bp->dead_end)
			continue;
		if (!(target & BACKEND_TYPE_NORMAL) && !bp->is_autovacuum && !bp->is_siasgc && !bp->is_siaspers)
			continue;
		if (!(target & BACKEND_TYPE_AUTOVAC) && bp->is_autovacuum && !bp->is_siasgc && !bp->is_siaspers)
			continue;
		if (!(target & BACKEND_TYPE_SIASGC) && !bp->is_autovacuum && bp->is_siasgc && !bp->is_siaspers)
			continue;
		if (!(target & BACKEND_TYPE_SIASPERS) && !bp->is_autovacuum && !bp->is_siasgc && bp->is_siaspers)
			continue;
...
sigusr1_handler(SIGNAL_ARGS)
{
	...
	if (CheckPostmasterSignal(PMSIGNAL_START_SIASGC_WORKER))
	{
		StartSiasGarbageCollectionWorker();
	}
...
static void
StartSiasGarbageCollectionWorker(void)
{
	Backend    *bn;
...
static void
StartSiasPersistenceWorker(bool startup)
{
	Backend    *bn;
...
//	if (canAcceptConnections() == CAC_OK)
	/*
	 * We'll be careful not to call StartSiasPersistenceWorker()
	 * if it's not possible to fork a new backend.
	 *
	 * We can't use the usual condition, because we need
	 * SiasPers worker before and after PM_RUN state.
	 */
	if (true)
	{
		bn = (Backend *) malloc(sizeof(Backend));
		if (bn)
		{
			...
			bn->pid = StartSiasPersWorker(startup);
			SiasPersPID = bn->pid;
			if (bn->pid > 0)
			{
				bn->is_autovacuum = false;
